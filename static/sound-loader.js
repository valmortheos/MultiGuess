const SoundLoader = (function() {
    ...
    function readChunkWithTimeout(reader, timeoutMs = 8000) {
        return Promise.race([
            reader.read(),
            new Promise((_, reject) =>
                setTimeout(() => reject(new Error('Chunk read timeout')), timeoutMs)
            )
        ]);
    }

    async function loadSoundsInternal() {
        ...
        for (const fileInfo of manifest.files) {
            ...
            try {
                ...
                if (reader) {
                    while (true) {
                        if (isSkipped) break;
                        const { done, value } = await readChunkWithTimeout(reader, 8000);
                        if (done) break;
                        chunks.push(value);
    ...
    async function loadSounds() {
        initUI();
        try {
            // Overall 25-second timeout for audio loading process
            await Promise.race([
                loadSoundsInternal(),
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Total sound loading timeout (25s exceeded)')), 25000)
                )
            ]);
        } catch (err) {
